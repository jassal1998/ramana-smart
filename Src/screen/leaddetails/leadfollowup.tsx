...
import { fetchData } from "../redux/slices/Leadfolloe/leadfollowthunk";
...
const [isLoading, setIsLoading] = useState(false);
 ...
 const filteredData = data.filter((item) => {
  ...
  const matchesFollowUpCall =
    showFollowUpCalls ? item.leadphase.includes('follow_up_call') : false;

  const matchesColdCall =
    showColdCalls ? item.leadphase.includes('cold_call') : false;

  const matchesAll =
    showAllData || matchesFollowUpCall || matchesColdCall;
...
<KeyboardAwareScrollView
    ...
    <TouchableOpacity    onPress={()=>navigation.navigate('Leaddetail')} style={style.create}>
      <Text style={{textAlign:'center',color:"white",fontSize:RFValue(12)}} adjustsFontSizeToFit={true}>Create Lead</Text></TouchableOpacity>
...
<View style={style.all}> 
    <TouchableOpacity style={style.button}   onPress={() => {
      ...
      <Text style={{textAlign:'center',color:"white",fontSize:RFPercentage(1.60)}}>All</Text></TouchableOpacity>
<TouchableOpacity style={style.Follow}   onPress={() => {
            setShowAllData(false);
            setShowFollowUpCalls(true);
            setShowColdCalls(false);
          }}>
  <Text style={{textAlign:'center',color:'white',fontSize:RFPercentage(1.60)}}>Follow Up Call</Text></TouchableOpacity>
<TouchableOpacity style={style.cold}  onPress={() => {
            setShowColdCalls(true);
            setShowFollowUpCalls(false);
            setShowAllData(false);
          }}>
  <Text style={{textAlign:'center',color:'white',fontSize:RFPercentage(1.60)}}>Cold Call</Text></TouchableOpacity>
</View>
...
const style = StyleSheet.create({
    follow:{paddingTop:10,flexDirection:'row',justifyContent:'space-between'},
